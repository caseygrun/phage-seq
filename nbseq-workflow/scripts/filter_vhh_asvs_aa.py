...
with snakemake_log(snakemake) as logfile:
	...
	cdrs = pd.read_csv(snakemake.input['cdrs'])
	print("Available segments:")
	print(cdrs.columns)

	cdr_lens = nbseq.tidy.segment_lens(cdrs, min_cdr_lens.keys())

	asvs = pd.read_csv(snakemake.input['aa'])
	asvs['length'] = asvs['aligned'].apply(nbseq.utils.len_ungapped)

	df = pd.merge(left=cdr_lens, right=asvs, on='aaSVID')
	...
	keep = df.query(query).loc[:,'aaSVID']
	...
	asvs.loc[asvs['aaSVID'].isin(keep),:].to_csv(snakemake.output['aa'],index=False)
	cdrs.loc[cdrs['aaSVID'].isin(keep),:].to_csv(snakemake.output['cdrs'],index=False)
